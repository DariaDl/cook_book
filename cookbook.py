    ...
    for dish in dishes:
        for ingridient in ing_dict[dish]:
            if ingridient["ingridient_name"] not in list_by_dishes.keys():
                list_by_dishes[ingridient["ingridient_name"]] = {"measure": ingridient["measure"],
                                                                 "quantity": ingridient["quantity"] * person_count}
            else:
                list_by_dishes["quantity"] = ingridient["quantity"] * person_count
...
print(get_shop_list_by_dishes(('Запеченный картофель', 'Омлет'), 2, creating_dict('df'))
)
